...
def get_jugadores_partido():
    response = supabase.table("jugadores_partido").select("*, jugador_id:jugadores(nombre), partido_id:partidos(fecha)").execute()

    if getattr(response, "error", None):
        raise HTTPException(status_code=400, detail=f"Error al obtener los JugadoresPartido: {response.error.message}")
...
def eliminar_jugador_partido(id: int):
    response = supabase.table("jugadores_partido").delete().eq("id", id).execute()
...
def actualizar_jugador_partido(id: int, jugador_partido: JugadorPartidoUpdate):
    ...
    jugador_partido_existente = supabase.table("jugadores_partido").select("id").eq("id", id).execute()
    if not jugador_partido_existente.data:
        raise HTTPException(status_code=404, detail="JugadorPartido no encontrado")

    response = supabase.table("jugadores_partido").update(datos_actualizados).eq("id", id).execute()
