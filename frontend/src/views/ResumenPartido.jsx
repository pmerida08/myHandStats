...
import { FaBars, FaFutbol, FaBullseye, FaTimes, FaSync, FaExclamationTriangle, FaSquareFull, FaUndo,FaHandPaper, FaSquare } from "react-icons/fa";
import {
  Box,
  Flex,
  Text,
...
const ResumenPartido = () => {
  ...
  useEffect(() => {
    ...
    const fetchDatos = async () => {
      try {
        ...
        const [partidoRes, accionesRes, tiposRes, jugadoresRes, jugadoresEquipoRes] = await Promise.all([
          fetch(`https://myhandstats.onrender.com/equipo/${equipo_id}/partido/${partido_id}`, { headers }),
          fetch(`https://myhandstats.onrender.com/equipo/${equipo_id}/partido/${partido_id}/acciones_partido`, { headers }),
          fetch(`https://myhandstats.onrender.com/acciones`, { headers }),
          fetch(`https://myhandstats.onrender.com/equipo/${equipo_id}/partido/${partido_id}/jugadores_partido`, { headers }),
          fetch(`https://myhandstats.onrender.com/equipo/${equipo_id}/jugadores`, { headers })
        ]);
        ...
        const jugadoresConNombre = jugadoresData.map(jp => {
          const jugadorInfo = jugadoresEquipo.find(j => j.id === jp.jugadores_id);
          return {
            ...jp,
  ...
  const getIdsPorTipo = (tipo) => tiposAcciones.filter(a => a.tipo_accion === tipo).map(a => a.id);
  ...
  const formatearAccion = (id) => {
    ...
    if (tipo === "goles") return <><FaFutbol style={{ display: "inline" }} /> Gol</>;
    if (tipo === "lanzamiento") return <><FaBullseye style={{ display: "inline" }} /> Lanzamiento</>;
    if (tipo === "perdida") return <><FaTimes style={{ display: "inline" }} /> Pérdida</>;
    if (tipo === "recuperacion") return <><FaSync style={{ display: "inline" }} /> Recuperación</>;
    if (tipo === "gol_en_contra") return <><FaExclamationTriangle style={{ display: "inline" }} /> Gol en Contra</>;
    if (tipo === "amonestacion") {
      const mapa = {
        tarjetas_amarillas: { icon: <FaSquareFull color="#facc15" />, label: "Amarilla" },
        tarjetas_rojas: { icon: <FaSquareFull color="#f87171" />, label: "Roja" },
        tarjetas_azules: { icon: <FaSquareFull color="#60a5fa" />, label: "Azul" },
        exclusion_2_min: { icon: <FaExclamationTriangle />, label: "2 min" },
      };
      const item = mapa[nombre];
      return item ? <>{item.icon} {item.label}</> : <>⚠️ Amonestación</>;
    }
    return <>📌 {nombre}</>;
  };
  ...
  const lanzamientosExtra = acciones.filter(a => lanzamientosIds.includes(a.acciones_id));
  const perdidasEquipo = acciones.filter(a => perdidasIds.includes(a.acciones_id));

  const totalLanzamientos = golesEquipo.length + lanzamientosExtra.length;
  const eficaciaEquipo = totalLanzamientos > 0 ? Math.round((golesEquipo.length / totalLanzamientos) * 100) : 0;
  ...
  const ordenarAccionesPorTiempo = (acciones) => {
    ...
    return [...acciones].sort((a, b) => toSegundos(a.minuto) - toSegundos(b.minuto));
  };
    ...
    <Flex>
      <Box flex="1" bg="white" p={6}>
        ...
        {tabSeleccionado === "Resumen" && (
          <>
            ...
            <Box mb={6}>
              ...
              <Flex h="8px" bg="#f0f0f0" overflow="hidden">
                <Box w={`${(perdidasEquipo.length / 20) * 100}%`} bg="teal.500" />
              </Flex>
            </Box>
        ...
        {tabSeleccionado === "Goleadores" && (
          <Box>
            <Text fontSize="lg" fontWeight="semibold" textAlign="center" mb={4}>Goleadores del Partido</Text>
            <TableContainer>
        ...
        {tabSeleccionado === "Timeline" && (
          <Box>
            <Text fontSize="lg" fontWeight="semibold" textAlign="center" mb={6}>
              Timeline del Partido
            </Text>

            <VStack spacing={8} align="stretch" position="relative">
              <Box position="absolute" top={0} bottom={0} left="50%" w="2px" bg="green.400" zIndex={0} />

              {ordenarAccionesPorTiempo(acciones).map((accion, index) => {
                const esGolEnContra = golesContraIds.includes(accion.acciones_id);
                const tipo = tiposAcciones.find(t => t.id === accion.acciones_id)?.tipo_accion;
                const nombre = tiposAcciones.find(t => t.id === accion.acciones_id)?.nombre;
                ...
                const iconos = {
                  ...
                  tarjetas_amarillas: <FaSquare style={{ color: '#ECC94B' }} />,
                ...
                const icono = iconos[nombre] || iconos[tipo] || <FaRegQuestionCircle />;

                return (
                  <Flex key={index} justify={esGolEnContra ? "flex-end" : "flex-start"} position="relative" zIndex={1}>
                    <Box w="50%" pr={esGolEnContra ? 0 : 6} pl={esGolEnContra ? 6 : 0}>
                      <HStack
                        spacing={3}
                        px={4}
                        py={2}
                      ...
                      >
                        <Box>
                          <Text fontWeight="semibold" fontSize="sm" color="gray.800">
                            {nombre?.replaceAll("_", " ")?.replace("tarjetas", "").toLowerCase()}
                          </Text>
                          <Text fontSize="xs" color="gray.500">{accion.minuto}'</Text>
                        </Box>
                        {icono}
                      </HStack>
                    </Box>
        ...
        {tabSeleccionado === "Jugadores" && (
          <Box>
            <Text fontSize="lg" fontWeight="semibold" textAlign="center" mb={4}>Jugadores del Partido</Text>
            <TableContainer>
              <Table variant="striped">
                <Thead>
                  <Tr>
